...
$conditions = ["buku.id_buku >= 5"];

if ($idKategoriTerpilih) {
    $conditions[] = "buku.id_kategori = $idKategoriTerpilih";
}

if (!empty($keyword)) {
    $escapedKeyword = $db->real_escape_string($keyword);
    $conditions[] = "buku.judul_buku LIKE '%$escapedKeyword%'";
}
...
if (!empty($conditions)) {
    $querySemua .= " WHERE " . implode(" AND ", $conditions);
}

$querySemua .= " ORDER BY buku.id_buku ASC LIMIT 4";
